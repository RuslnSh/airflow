...
def _generate_events(end_date):
    ...
    events = pd.concat(
        [
            _generate_events_for_day(date=end_date - timedelta(days=(i+1)))
            for i in range(30)
        ],
        axis=0,
    )
...
app = Flask(__name__)
app.config["events"] = _generate_events(end_date=datetime.now().date())


@app.route("/events")
def events():
    # print("start_date" + str(request.args.get("start_date", None)))
    # print("end_date" + str(request.args.get("end_date", None)))
